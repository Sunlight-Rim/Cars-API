...
var errorWord = map[string]string{
	"Inn":    "INN",
	"Ogrnip": "OGRNIP",
	"Ogrn":   "OGRN",
}
...
func main() {
	...
	if err := saveFile(
		fmt.Sprintf("%s.go", fileName), // filename
		formatedErrWebJson,             // data
	); err != nil {
		log.Fatalf("failed save %s.go: %v", fileName, err)
	}
}

func saveFile(filename string, data []byte) error {
	if err := os.WriteFile(filepath.Join(getDir(), "pkg/errors", filename), data, 0644); err != nil {
		return fmt.Errorf("write in file=%s: %v", filename, err)
	}
...
func parseErrJson() ([]errorJson, error) {
	data, err := os.ReadFile(filepath.Join(getDir(), "", "configs/errors.json"))
	if err != nil {
...
func generateErrVar(str string) string {
	words := strings.Split(strings.Title(strings.ToLower(str)), " ")

	for i := range words {
		if word, ok := errorWord[words[i]]; ok {
			words[i] = word
		}
	}
